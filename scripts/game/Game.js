...
class Game {
  ...
  initGame(difficulty) {
    ...
    this.game_animation = window.requestAnimationFrame(this.draw_bind)
  ...
  gameOverAnimation(last_hitted_enemy_id) {
    ...
    for (let i = 0; i < Enemy.amount; i++) {
      ...
      }, 250 * (i - enemies_skipped + (i < last_hitted_enemy_id)))
    }
  }
  ...
  newEnemyPosition(enemy_id) {
    ...
    while (position_ok < enemies_length - 1 && attempts < max_attempts) {
      attempts++;
      ...
      for (let i = 0; i < enemies_length; i++) {
        ...
        if (!this.isEnemyTheSameColumn(this.enemies[enemy_id], this.enemies[i])) {
          position_ok++
          continue
        }
    ...
    if (position_ok < enemies_length - 1) return
    this.enemies[enemy_id].is_ready = true
  }

  isEnemyTheSameColumn(new_enemy, old_enemy) {
