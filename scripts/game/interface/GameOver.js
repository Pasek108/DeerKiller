...
class GameOver {
  ...
  show() {
    this.html.style.pointerEvents = "all"
  ...
  saveButtonClick() {
    if (this.save_button_state === 0) {
      ...
      pts_counter.copyPoints(this.game_object.points_counter.points)
    } else if (this.save_button_state === 1) {
      if (this.checkNickname() && this.checkPoints()) {
        const name = this.text.querySelector("input").value
        this.sendScore(name, this.game_object.points_counter.points)
      }
  ...
  sendScore(name, points) {
    ...
    const records = localStorage.getItem("deerkiller-" + level)
    ...
    localStorage.setItem("deerkiller-" + level, data.join(";"))
  ...
  backButtonClick() {
    if (!Menu.sound_muted) menu.menu_music.play()
    menu.menu_music.volume = 0
    this.game_object.resetGame()
    menu.top_score.activateScoreTab(0)
    fadeIn(menu.black_screen)
    let id = setInterval(() => {
      if (menu.menu_music.volume > 0.98) {
        menu.menu_music.volume = 1
        this.reset()
        fadeIn(menu.container)
        fadeIn(menu.difficulty.container)
        clearInterval(id)
      } else menu.menu_music.volume += 0.01
    }, 20)
  }
