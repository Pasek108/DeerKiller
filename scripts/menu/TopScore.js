...
class TopScore extends MenuWindow {
  constructor(class_name, position, menu_container) {
    ...
    if (localStorage.getItem("deerkiller-easy") === null) localStorage.setItem("deerkiller-easy", "")
    if (localStorage.getItem("deerkiller-medium") === null) localStorage.setItem("deerkiller-medium", "")
    if (localStorage.getItem("deerkiller-hard") === null) localStorage.setItem("deerkiller-hard", "")
    if (localStorage.getItem("deerkiller-hardcore") === null) localStorage.setItem("deerkiller-hardcore", "")
  ...
  fillPersonalTop10(level) {
    ...
    const string_data = localStorage.getItem("deerkiller-" + level_name)
    if (string_data === "") {
  ...
  fillGeneralTop10(level) {
    ...
    const empty = document.createElement("span")
    empty.className = "empty flex-center-column"
    empty.innerText = "Not available (I couldn't find free hosting)"
